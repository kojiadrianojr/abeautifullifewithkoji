 ...
 * 
 * Usage:
 *   node scripts/sync-rsvp.js [spreadsheetId]
 * 
...
async function main() {
	try {
		// Import the GoogleFormsService (server-side only)
		const googleFormsModule = await import('../src/services/googleFormsService.js');
		const { GoogleFormsService } = googleFormsModule;
		...
		if (!spreadsheetId) {
			...
			console.log('\nUsage:');
			console.log('  node scripts/sync-rsvp.js [spreadsheetId]');
